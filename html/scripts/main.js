...
function printer(inputClassName, outputClassName, btnName, spanID, spanClass){
	...
	for (var i = 0; i < paragraphs.length; i++){
		...
		if (spanItem.length != 0) {
			...
			setTimeout(function(item){item.style.display='block'}, 200*t, prints[i]);
			t += 1;
		} else {
			t = wordByWordWithOut(t, paragraphs[i], prints[i]);
		}
	}
	setTimeout(function(item){item.style.opacity='1'}, 200*(t+1), document.getElementById(btnName));
}
// without span
function wordByWordWithOut(t0, para, print) {
	const data = para.innerHTML.split('');
	for (var i = 0; i < data.length; i++) {
		setTimeout(function(container, word){container.innerHTML += word}, 200*(t0+i), print, data[i]);
	}
	return t0+i+1
}
// with span
	...
	for (var i = 0; i < elements.length; i++) {
		if (i==0 || i==2) {
			...
			for (var j = 0; j < data.length; j++) {
				setTimeout(function(container, word){container.innerHTML += word}, 200*(t0+i+j+k), print, data[j]);
			}
		} else {
			...
			setTimeout(function(container, word){container.appendChild(word)}, 200*(t0+i+k), print, temp);
			j=0;
		}
		k += j;}
	return t0+i+k
